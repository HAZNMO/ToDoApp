...
async def get_user_todos(user_todos: TodoList):
    ...
    if user_todos.task_status is None:
        task_status = await todo_collection.find({"user_id": user_todos.user_id}).to_list(1000)
        return task_status
...
async def update_user_todo(update_todos: UpdateTODOModel):
    update_data = {k: v for k, v in update_todos.model_dump(by_alias=True).items() if v is not None}
